    ...
    <div style={{ minHeight: "100vh", backgroundColor: C.white, fontFamily: C.sans, color: C.bodyText }}>
      <style>{`
        * { box-sizing: border-box; }
        ...
        @media(max-width:900px){ .blog-hero-grid { grid-template-columns:1fr; } }
        .hero-right { display:block; }
        @media(max-width:900px){ .hero-right { display:none; } }
        .hero-overlay { position:absolute; inset:0; background:linear-gradient(to right, rgba(12,35,64,0.92) 55%, rgba(12,35,64,0.60) 100%); }
        @media(max-width:768px){ .hero-overlay { background:rgba(12,35,64,0.88); } }
        .hero-cta-row { display:flex; gap:12px; flex-wrap:wrap; }

        /* Stat cards */
        .stat-card { background:rgba(255,255,255,0.1); border:1px solid rgba(255,255,255,0.18); border-radius:14px; padding:16px 18px; backdrop-filter:blur(6px); }
        .stat-cards-grid { display:grid; grid-template-columns:1fr 1fr; gap:12px; margin-bottom:14px; }

        /* Category filter bar */
        .cat-filter-bar { background:${C.white}; border-bottom:1px solid ${C.border}; padding:0 24px; position:sticky; top:68px; z-index:10; box-shadow:0 2px 8px rgba(0,0,0,0.04); }
        ...
        .cat-btn { padding:7px 16px; border-radius:999px; font-size:13px; font-weight:500; border:1.5px solid ${C.border}; background:transparent; color:${C.mutedText}; cursor:pointer; white-space:nowrap; font-family:${C.sans}; transition:all 0.2s; flex-shrink:0; }
        .cat-btn:hover { border-color:${C.primary}; color:${C.primary}; }
        .cat-btn.active { background:${C.primary}; border-color:${C.primary}; color:#fff; }

        /* Featured post */
        .featured-card { border-radius:20px; overflow:hidden; margin-bottom:48px; display:grid; grid-template-columns:1fr 1fr; min-height:300px; border:1.5px solid ${C.border}; box-shadow:0 8px 32px rgba(0,0,0,0.08); }
        @media(max-width:768px){ .featured-card { grid-template-columns:1fr; } .featured-img { min-height:200px; } }
        .featured-content { padding:32px 36px; background:${C.navy}; display:flex; flex-direction:column; justify-content:center; }
      ...
      {/* ── HERO ── */}
      <section style={{ position: "relative", minHeight: "clamp(420px,70vh,640px)", display: "flex", alignItems: "center", overflow: "hidden" }}>
        <img
        ...
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "clamp(40px,8vw,80px) clamp(16px,4vw,40px)", width: "100%", position: "relative", zIndex: 2 }}>
          <div className="blog-hero-grid">
            ...
            <div>
              ...
              <h1 style={{ fontFamily: C.serif, fontSize: "clamp(1.8rem,4vw,3.4rem)", color: "#fff", margin: "0 0 16px", fontWeight: 800, lineHeight: 1.15 }}>
                Regulatory Updates &<br />Certification Guides
              </h1>
              <p style={{ color: "rgba(255,255,255,0.75)", fontSize: "clamp(13px,2vw,16px)", lineHeight: 1.8, marginBottom: 28, maxWidth: 500 }}>
              ...
              <div className="hero-cta-row">
                <button
                  ...
                  style={{ padding: "12px 26px", backgroundColor: C.primary, color: "#fff", fontWeight: 700, borderRadius: 12, border: "none", fontSize: 14, cursor: "pointer", fontFamily: C.sans, boxShadow: "0 4px 16px rgba(249,115,22,0.4)" }}
                ...
                <button
                  ...
                  style={{ padding: "12px 22px", border: "1.5px solid rgba(255,255,255,0.3)", color: "#fff", borderRadius: 12, background: "rgba(255,255,255,0.08)", fontSize: 14, cursor: "pointer", fontFamily: C.sans }}
              ...
              <div className="stat-cards-grid">
                {heroStats.map(s => (
                  <div key={s.label} className="stat-card">
                    ...
                    <div style={{ fontFamily: C.serif, fontSize: 22, color: C.primary, fontWeight: 800, marginBottom: 4 }}>{s.value}</div>
                    <div style={{ fontSize: 11, color: "rgba(255,255,255,0.65)" }}>{s.label}</div>
                  </div>
                ))}
              </div>
              <div style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 14, padding: "16px 18px", backdropFilter: "blur(6px)" }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: C.primary, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 10 }}>Topics Covered</div>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {["BIS", "EPR", "WPC", "TEC", "BEE", "LMPC", "ISO", "CDSCO", "QCO", "FSSAI"].map(t => (
                    <span key={t} style={{ padding: "4px 10px", borderRadius: 999, fontSize: 11, fontWeight: 600, background: "rgba(249,115,22,0.15)", color: C.primary, border: "1px solid rgba(249,115,22,0.25)" }}>{t}</span>
                  ))}
                </div>
              </div>
            </div>
      ...
      <section id="posts-section" className="sec-pad" style={{ background: C.offWhite }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          ...
          {activeCategory === "All" && (
            <div className="featured-card">
              ...
              <div className="featured-content">
                ...
                <h2 style={{ fontFamily: C.serif, fontSize: "clamp(1.1rem,2vw,1.6rem)", color: "#fff", marginBottom: 12, fontWeight: 800, lineHeight: 1.3 }}>{featured.title}</h2>
                <p style={{ fontSize: 13, color: "rgba(255,255,255,0.65)", lineHeight: 1.75, marginBottom: 20 }}>{featured.excerpt}</p>
                <div style={{ display: "flex", gap: 16, alignItems: "center", flexWrap: "wrap" }}>
                  <button style={{ padding: "10px 22px", backgroundColor: C.primary, color: "#fff", fontWeight: 700, borderRadius: 10, border: "none", fontSize: 13, cursor: "pointer", fontFamily: C.sans, boxShadow: "0 4px 12px rgba(249,115,22,0.35)" }}>
                    Read Full Article →
                  </button>
          ...
          <div style={{ marginBottom: 28 }}>
            ...
            <h2 style={{ fontFamily: C.serif, fontSize: "clamp(1.3rem,2.5vw,2rem)", color: C.navy, fontWeight: 800 }}>
              ...
              return (
                <div key={post.title} className="post-card">
                  ...
                  <div style={{ padding: "20px 22px", flex: 1, display: "flex", flexDirection: "column" }}>
                    <div style={{ fontSize: 11, color: C.mutedText, marginBottom: 8 }}>{post.date}</div>
                    <h3 style={{ fontFamily: C.serif, fontSize: 15, color: C.navy, marginBottom: 8, fontWeight: 700, lineHeight: 1.4, flex: 1 }}>{post.title}</h3>
                    <p style={{ fontSize: 13, color: C.mutedText, lineHeight: 1.6, marginBottom: 14 }}>{post.excerpt}</p>
      ...
      {/* ── NEWSLETTER ── */}
      <section id="newsletter-section" style={{ position: "relative", padding: "clamp(56px,8vw,80px) 24px", overflow: "hidden" }}>
        <img
        ...
        <div style={{ maxWidth: 560, margin: "0 auto", textAlign: "center", position: "relative", zIndex: 1 }}>
          ...
          <h2 style={{ fontFamily: C.serif, fontSize: "clamp(1.4rem,3vw,2.2rem)", color: "#fff", marginBottom: 12, fontWeight: 800 }}>
            Get Regulatory Updates in Your Inbox
          </h2>
          <p style={{ fontSize: "clamp(13px,2vw,14px)", color: "rgba(255,255,255,0.65)", marginBottom: 28, lineHeight: 1.7 }}>
            New QCO notifications, BIS updates, EPR changes — delivered weekly. No spam, ever.
          </p>
          <div className="newsletter-input-row">
            <input type="email" placeholder="Your email address"
              style={{ flex: 1, padding: "13px 16px", border: "1.5px solid rgba(255,255,255,0.2)", borderRadius: 10, fontSize: 14, outline: "none", fontFamily: C.sans, backgroundColor: "rgba(255,255,255,0.1)", color: "#fff", backdropFilter: "blur(4px)", minWidth: 0 }} />
            <button
              style={{ padding: "13px 22px", backgroundColor: C.primary, color: "#fff", fontWeight: 700, borderRadius: 10, border: "none", fontSize: 14, cursor: "pointer", fontFamily: C.sans, whiteSpace: "nowrap", boxShadow: "0 4px 14px rgba(249,115,22,0.4)" }}
              onMouseEnter={e => e.currentTarget.style.backgroundColor = C.primaryDark}
              onMouseLeave={e => e.currentTarget.style.backgroundColor = C.primary}
            >Subscribe →</button>
          </div>
          <p style={{ fontSize: 12, color: "rgba(255,255,255,0.4)", marginTop: 12 }}>Join 5,000+ professionals. Unsubscribe anytime.</p>
        </div>
      </section>
