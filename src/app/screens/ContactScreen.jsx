    ...
    <div style={{ minHeight: "100vh", backgroundColor: C.white, fontFamily: C.sans, color: C.bodyText }}>
      <style>{`
        * { box-sizing: border-box; }
        ...
        /* Hero */
        .hero-overlay { position:absolute; inset:0; background:linear-gradient(to right, rgba(12,35,64,0.90) 55%, rgba(12,35,64,0.50) 100%); }
        @media(max-width:768px){ .hero-overlay { background:rgba(12,35,64,0.88); } }
        ...
        .hero-badge-top { position:absolute; top:24px; right:24px; z-index:3; background:rgba(255,255,255,0.12); backdrop-filter:blur(10px); border:1px solid rgba(255,255,255,0.2); border-radius:14px; padding:10px 16px; display:flex; gap:14px; }
        @media(max-width:560px){ .hero-badge-top { display:none; } }
        ...
        /* Form */
        .form-card { background:${C.white}; border-radius:20px; padding:36px; border:1.5px solid ${C.border}; box-shadow:0 4px 24px rgba(0,0,0,0.06); }
        ...
        .input-field:focus { border-color:${C.primary}; background:${C.white}; }
        ...
        .office-card { background:${C.white}; border-radius:16px; padding:28px; border:1.5px solid ${C.border}; transition:all 0.25s; display:flex; flex-direction:column; justify-content:space-between; }
        .office-card:hover { border-color:${C.primary}; transform:translateY(-3px); box-shadow:0 12px 32px rgba(249,115,22,0.12); }

        .office-image-panel { position:relative; border-radius:20px; overflow:hidden; min-height:400px; }
        @media(max-width:900px){ .office-image-panel { min-height:300px; } }

        .office-stats-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:14px; margin-top:32px; }
        @media(max-width:480px){ .office-stats-grid { grid-template-columns:1fr; gap:10px; margin-top:20px; } }
      ...
      {/* ── HERO ── */}
      <section style={{ position: "relative", minHeight: "clamp(480px,70vh,600px)", display: "flex", alignItems: "center", overflow: "hidden" }}>
        <img
        ...
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "clamp(40px,8vw,80px) clamp(16px,4vw,40px)", width: "100%", position: "relative", zIndex: 2 }}>
          <div className="hero-grid">
            ...
            <div>
              ...
              <h1 style={{ fontFamily: C.serif, fontSize: "clamp(1.8rem,4vw,3.2rem)", color: "#fff", lineHeight: 1.15, marginBottom: 18, fontWeight: 800 }}>
                Let's Start Your{" "}
                <span style={{ color: C.primary }}>Certification</span>{" "}
                Journey
              </h1>
              <p style={{ fontSize: "clamp(13px,2vw,15px)", lineHeight: 1.8, color: "rgba(255,255,255,0.8)", marginBottom: 32, maxWidth: 480 }}>
              ...
              <div className="hero-cta-row">
                <a href="#contact-form"
                  style={{ padding: "13px 28px", backgroundColor: C.primary, color: "#fff", fontWeight: 700, borderRadius: 12, textDecoration: "none", fontSize: 14, boxShadow: "0 6px 20px rgba(249,115,22,0.45)" }}>
                  Send a Message ↓
                </a>
                <a href="tel:+919540190334"
                  style={{ padding: "13px 22px", border: "2px solid rgba(255,255,255,0.4)", color: "#fff", borderRadius: 12, textDecoration: "none", fontSize: 14, fontWeight: 600, backdropFilter: "blur(4px)" }}>
                  📞 Call Us Now
                </a>
              </div>
              <div className="hero-trust-badges">
                {["✓ Free Consultation", "✓ 2-Hr Response", "✓ 98% Success Rate", "✓ Pan-India"].map(b => (
                  <span key={b} style={{ padding: "5px 12px", border: "1px solid rgba(255,255,255,0.25)", borderRadius: 999, fontSize: 12, color: "rgba(255,255,255,0.85)", background: "rgba(255,255,255,0.08)" }}>{b}</span>
                ))}
              </div>
            </div>
            ...
            <div className="hero-right">
              ...
              <div className="trust-grid">
                {heroTrust.map(t => (
                  <div key={t.label} style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(249,115,22,0.25)", borderRadius: 14, padding: "16px 14px", display: "flex", gap: 10, alignItems: "flex-start" }}>
                    <div style={{ fontSize: 20, flexShrink: 0, lineHeight: 1 }}>{t.icon}</div>
                    <div>
                      <div style={{ fontFamily: C.serif, fontSize: 13, color: "#fff", fontWeight: 700, marginBottom: 3 }}>{t.label}</div>
                      <div style={{ fontSize: 11, color: "rgba(255,255,255,0.65)", lineHeight: 1.5 }}>{t.desc}</div>
                    </div>
                  </div>
                ))}
                <div style={{ gridColumn: "span 2", background: "rgba(249,115,22,0.15)", border: "1px solid rgba(249,115,22,0.3)", borderRadius: 14, padding: "12px 16px", display: "flex", alignItems: "center", gap: 12 }}>
                  <span style={{ fontSize: 20, flexShrink: 0 }}>🏢</span>
                  <div>
                    <div style={{ fontSize: 13, color: "#fff", fontWeight: 600, marginBottom: 2 }}>Based in New Delhi</div>
                    <div style={{ fontSize: 11, color: "rgba(255,255,255,0.65)" }}>North West Delhi · Serving all of India</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <div className="stats-strip">
            {contactStats.map((s, i) => (
              <div key={s.label} style={{ textAlign: "center", padding: "26px 16px", borderRight: i < contactStats.length - 1 ? "1px solid rgba(255,255,255,0.08)" : "none" }}>
      ...
      <section id="contact-form" className="sec-pad" style={{ background: C.white }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <div className="contact-main-grid">
            ...
            <div className="form-card">
              {/* Banner */}
              <div style={{ position: "relative", borderRadius: 14, overflow: "hidden", height: 150, marginBottom: 28 }}>
                <img src="https://images.unsplash.com/photo-1521737711867-e3b97375f902?w=900&q=80&fit=crop"
                ...
                <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", padding: "0 24px" }}>
                  <div>
                    <div style={{ fontFamily: C.serif, fontSize: "clamp(1rem,2vw,1.4rem)", color: "#fff", fontWeight: 800, marginBottom: 4 }}>Talk to a Regulatory Expert</div>
                    <p style={{ color: "rgba(255,255,255,0.72)", fontSize: 12 }}>We respond within 2 business hours. No spam, ever.</p>
                  </div>
                </div>
              </div>
              ...
              <div className="two-col">
                {[{ label: "Phone Number *", type: "tel", placeholder: "+91 98765 43210" }, { label: "Email Address *", type: "email", placeholder: "you@company.com" }].map(f => (
                  <div key={f.label}>
                    <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#1A1A2E", marginBottom: 6 }}>{f.label}</label>
                    <input type={f.type} placeholder={f.placeholder} className="input-field" />
                  </div>
              ...
              <button
                style={{ width: "100%", padding: "14px", backgroundColor: C.primary, color: C.white, fontWeight: 700, borderRadius: 12, border: "none", fontSize: 15, cursor: "pointer", fontFamily: C.sans, boxShadow: "0 6px 20px rgba(249,115,22,0.35)" }}
                onMouseEnter={e => e.currentTarget.style.backgroundColor = C.primaryDark}
                onMouseLeave={e => e.currentTarget.style.backgroundColor = C.primary}>
                Submit Enquiry →
              </button>
              <p style={{ fontSize: 12, color: C.mutedText, textAlign: "center", marginTop: 10 }}>We respond within 2 business hours. No spam, ever.</p>
            </div>
            ...
            <div className="sidebar">
              ...
              <div style={{ position: "relative", borderRadius: 16, overflow: "hidden" }}>
                <img src="https://images.unsplash.com/photo-1568219557405-376e23e4f7cf?w=600&q=80&fit=crop"
                  alt="Urgent" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }} />
                ...
                <div style={{ position: "relative", zIndex: 1, padding: 24 }}>
                  ...
                  <p style={{ fontSize: 13, color: "rgba(255,255,255,0.88)", lineHeight: 1.65, marginBottom: 16 }}>Facing a regulatory deadline or port hold? We have an emergency response team available 24/7.</p>
      ...
      {/* ── OFFICE ── */}
      <section className="sec-pad" style={{ backgroundColor: C.offWhite }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          ...
          <div className="office-split">
            ...
            <div className="office-card">
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
                  <div style={{ width: 48, height: 48, borderRadius: 14, backgroundColor: C.primaryLight, border: "1.5px solid #FED7AA", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, flexShrink: 0 }}>🏢</div>
                  <div>
              ...
              <div style={{ marginTop: 28, paddingTop: 22, borderTop: `1px solid ${C.border}` }}>
                ...
                <a href="https://maps.google.com/?q=Pocket+9+North+West+New+Delhi+110086" target="_blank" rel="noopener noreferrer"
                  style={{ display: "block", textAlign: "center", padding: "12px", border: `1.5px solid ${C.border}`, color: C.navy, borderRadius: 10, textDecoration: "none", fontSize: 14, fontWeight: 600 }}>
                  🗺️ Get Directions
                </a>
              </div>
            </div>
            ...
            <div className="office-image-panel">
              ...
              <div style={{ position: "relative", zIndex: 1, padding: "clamp(24px,4vw,40px)", height: "100%", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                <div>
                  <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(249,115,22,0.2)", border: "1px solid rgba(249,115,22,0.4)", borderRadius: 999, padding: "6px 14px", marginBottom: 18 }}>
                    <span style={{ width: 7, height: 7, borderRadius: "50%", backgroundColor: C.primary, display: "inline-block" }} />
                    <span style={{ color: C.primary, fontSize: 11, fontWeight: 600 }}>🇮🇳 New Delhi, India</span>
                  </div>
                  <h3 style={{ fontFamily: C.serif, fontSize: "clamp(1.2rem,2.5vw,2rem)", color: "#fff", fontWeight: 800, marginBottom: 12, lineHeight: 1.25 }}>
                    Your Trusted Partner<br />in New Delhi
                  </h3>
                  <p style={{ color: "rgba(255,255,255,0.75)", fontSize: "clamp(12px,1.5vw,14px)", lineHeight: 1.8, maxWidth: 420 }}>
                ...
                <div className="office-stats-grid">
                  {[
                    ...
                    <div key={s.label} style={{ background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 14, padding: "14px 12px", backdropFilter: "blur(8px)" }}>
                      <div style={{ fontFamily: C.serif, fontSize: "clamp(1rem,2vw,1.4rem)", color: C.primary, fontWeight: 800 }}>{s.value}</div>
                      <div style={{ fontSize: 11, color: "rgba(255,255,255,0.65)", marginTop: 4, lineHeight: 1.4 }}>{s.label}</div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <section style={{ position: "relative", padding: "clamp(56px,8vw,88px) 24px", overflow: "hidden" }}>
        <img src="https://images.unsplash.com/photo-1600880292203-757bb62b4baf?w=1600&q=80&fit=crop"
          alt="Get certified" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", objectPosition: "center" }} />
        ...
        <div style={{ maxWidth: 800, margin: "0 auto", textAlign: "center", position: "relative", zIndex: 1 }}>
          ...
          <p style={{ color: "rgba(255,255,255,0.88)", marginBottom: 36, lineHeight: 1.75, fontSize: "clamp(14px,2vw,16px)" }}>
            Our experts are ready to help. Get a free consultation and know exactly what you need — at no cost.
          </p>
          <div className="cta-btn-row">
            <a href="#contact-form"
              style={{ padding: "14px 36px", backgroundColor: "#fff", color: C.primary, fontWeight: 800, borderRadius: 12, fontSize: 14, textDecoration: "none", boxShadow: "0 6px 24px rgba(0,0,0,0.15)" }}>
